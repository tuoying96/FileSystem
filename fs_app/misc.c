...
/** Declaration should be in stdio.h but is not on macos */
extern ssize_t getline(char** restrict linep, size_t* restrict linecapp,
                       FILE* restrict stream);

/** Declaration should be in string.h but is not on macos */
extern char *strdup(const char* str);

/** All FUSE file system functions accessed through operations structure. */
extern struct fuse_operations fs_ops;

/**  Disk block device */
struct blkdev *disk;

/** Fuse argument parser user data */
static struct fuse_parser_data {
    char *image_name;  /** fuse image name */
    int   cmd_mode;  /** command mode flag */
} parser_data;
...
static struct fuse_opt opts[] = {
        {"-image %s", offsetof(struct fuse_parser_data, image_name), 0},
        {"-cmdline", offsetof(struct fuse_parser_data, cmd_mode), 1},
        FUSE_OPT_END
};
 ...
 * @param pathbuf result path buffer must be PATH_MAX length
 ...
 * @return the new nnames value after normalization
 *   and the new path length
 */
...
int do_ls1(char *argv[])
{
    ...
    char pathdotbuf[PATH_MAX];
    strcpy(pathdotbuf, pathbuf);
    strcat(pathdotbuf, (pathbuf[strlen(pathbuf)-1] == '/') ? "." : "/.");
...
static int dashl_filler(void *buf, const char *name, const struct stat *sb, off_t off)
{
    ...
    if (S_ISLNK(sb->st_mode)) {
        ...
        if (fs_ops.readlink != NULL) {
            ...
            int status = fs_ops.readlink(fullpath, sympath, PATH_MAX);
            if (status == 0) {
...
{
    char path[PATH_MAX];
    mode_t mode = strtol(argv[0], NULL, 8);
...
static int do_readlink(char *argv[])
{
    ...
    char sympath[PATH_MAX];
    int status = fs_ops.readlink(path, sympath, PATH_MAX);
    if (status == 0) {
        fprintf(stdout, "%s\n", sympath);
    }
    return status;
...
/* block buffer for coping files */
static char *blkbuf;

/**
 * Get a file from localdir into filesystem
 *
...
/**
 * Get a file from localdir into file system with
 * same name.
 *
 * @param argv arg[0] is file name; file system name
 *   relative to current directory
...
/**
 * Put a file from filesystem to localdir
 *
...
static int do_put(char *argv[])
{
    ...
    // create file on local directory
...
/**
 * Put a file from filesystem to localdir with
 * same name.
 *
...
/** Implement dispatch table for commands */
static struct fuse_cmds cmds[] = {
        ...
        {"get", 2, do_get, "get <outside> <inside> - get a file from local directory into file system"},
        ...
        {"put", 2, do_put, "put <inside> <outside> - put a file from file system to local directory"},
...
/**
 * Main program for the Fuse file system utility
 * and interactive command interpreter.
 *
 * @param argc number of arguments including the program name
 * @param argv the arguments specifying the image file and mode
...
int main(int argc, char **argv)
{
    ...
    if (fuse_opt_parse(&args, &parser_data, opts, NULL) == -1){
        help();
        return 1;
    }

    if (parser_data.image_name == 0){
        fprintf(stderr, "You must provide an image\n");
        help();
        return 1;
    }

    char *file = parser_data.image_name;
    ...
    if (parser_data.cmd_mode) {  /* process interactive commands */
        fs_ops.init(NULL);
